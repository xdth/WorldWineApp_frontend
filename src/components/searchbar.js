...
class Searchbar extends React.Component {
  ...
  async handleSubmit(event) {
    ...
    // api call
    const response = await fetch(process.env.REACT_APP_API_URL + '/wines?filter[where][title][like]=%25'+ this.state.title + '%25&filter[where][country][like]='+ this.state.country + '%25&filter[limit]=500');
